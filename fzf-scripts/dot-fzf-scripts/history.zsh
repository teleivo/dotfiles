  ...
  case "$1" in
    dir)
      ...
      sqlite3 -cmd '.separator "" "\x1e"' "$__h_db" \
        "select command from history where cwd = '$cwd' and deleted_at is null group by command order by max(timestamp) desc" \
        | tr '\036' '\0'
      ;;
    global)
      sqlite3 -cmd '.separator "" "\x1e"' "$__h_db" \
        "select command from history where deleted_at is null group by command order by max(timestamp) desc" \
        | tr '\036' '\0'
      ;;
    delete)
      local cmd
...
_fzf_history_search() {
  ...
  _FZF_HIST_CWD="$cwd" fzf \
    ...
    --bind 'alt-d:execute-silent(echo -n {} | zsh '"$__h"' delete)+transform:[[ $FZF_BORDER_LABEL =~ dir ]] &&
